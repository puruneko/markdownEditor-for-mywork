...
### 依存
- `issue-phase001-005__cross-file-identity-and-viewmodel`（globalKey・`resolveRef`・`patchInFile`）。

### クロスファイル前提（phase001-005 準拠）
- DataTransfer のペイロードは **globalKey**（または `{sourcePath, localId}`）を載せる。書き戻しは `patchInFile(index, globalKey, ...)` で**そのアイテムの sourcePath のファイル**に対して行う（`getCurrentFile()` に書かない）。
- `findNodeById` は `patchInFile` 内で**単一ファイルの Document**に対してのみ呼ぶ。後述の upsert もそのファイル文字列に対して行う。
- ドロップ先パネルのアイテムキーは globalKey 前提（`phase001-005` のビューモデル）。
...
### TODO
- [x] 着手前スパイク（外部ドロップ／座標→日時 API の有無）を実施し、結果を履歴に記録。不可なら停止・報告。
- [x] メタ upsert（挿入＋置換）関数を新規実装。
- [x] CM6 ドラッグ源（選択→`nodeId` 逆引き→DataTransfer）。
- [x] Calendar + Kanban Mount に dragover/drop ＋ 座標→日時/列算出。
- [ ] ガント Mount（ライブラリ拡張待ち、別 issue: issue-phase002-004b 等で対応）。
- [x] `settings.ts` に `defaultDurationMin`。
- [x] テストを追加・全見直し（下記テスト観点）。

### ⚠️ スパイク結果（ガント未実装の理由）
ライブラリ調査の結果:
- **Calendar**: `.day-column` DOM + `storage.weekSettings`（`startHour`・`minorTick`）で座標→日時算出可能 ✅
- **Kanban**: `data-lane-id` 属性でドロップ先レーン→ステータス特定可能 ✅
- **Gantt**: `extendedDateRange`（ライブラリ内部 Svelte ストア、外部非公開）が座標→日時変換に必須。
  `calculateDateRange(nodes).start` はバッファ分（最大90日）ずれており近似不可。
  `AI_RUNTIME_RULES §4` に従い実装を停止。ライブラリへの `onExternalDrop(date)` コールバック追加を別 issue で提案すること。
...
- 2026-06-29 — 実装完了（Calendar + Kanban）。Gantt はライブラリ内部 `extendedDateRange` が非公開のためスパイク不可と判断、別 issue 提案。
  新規ファイル: `src/lib/patch/upsert-meta.ts`・`.test.ts`、`src/editor/task-drag-source.ts`。
  変更ファイル: `src/lib/calendar/CalendarTab.svelte`、`src/lib/kanban/KanbanTab.svelte`、`src/plugin.ts`、`src/settings.ts`。
  全ユニットテスト 422 件パス（新規 14 件含む）。
...
- related_decisions:
- target_files: src/editor/task-drag-source.ts, src/lib/patch/upsert-meta.ts, src/lib/calendar/CalendarTab.svelte, src/lib/kanban/KanbanTab.svelte, src/plugin.ts, src/settings.ts
- created: 2026-06-28
- updated: 2026-06-29
